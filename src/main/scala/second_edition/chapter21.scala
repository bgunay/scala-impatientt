...
object chapter21 extends App {
  ...
  implicit class RichFraction(private val self: Fraction) extends Ordered[Fraction] {
    override def compare(that: Fraction): Int = self.toDouble.compare(that.toDouble)
  }
  ...
  println(Fraction(1, 7) < Fraction(2, 9)) // true
  println(Fraction(1, 7) < Fraction(1, 9)) // false
  println(smaller(Fraction(1, 7), Fraction(2, 9))) // 1/7
  println(smaller(Fraction(1, 7), Fraction(1, 9))) // 1/9
 ...
 // println(new Point(3, 4) < new Point(4, 5)) // true
 // println(new Point(4, 5) < new Point(4, 4)) // false
  ...
  implicit class DistanceToOriginPointOrdering(private val self: Point)
    ...
    def distanceToOriginSquared: Int = self.x * self.x + self.y * self.y
  }
